...
namespace SCC.Modules.DotMap.Coord
{
    internal class OSRef 
    {
         ...
         * @since 1.0
         */
        public OSRef(String osref)
        { 
            // if (ref.matches(""))
            char char1 = osref[0];
            char char2 = osref[1];
            // Thanks to Nick Holloway for pointing out the radix bug here
            int east = System.Convert.ToInt32(osref.Substring(2, 3)) * 100;
            int north = System.Convert.ToInt32(osref.Substring(5, 3)) * 100;
            if (char1 == 'H')
            {
                north += 1000000;
            }
        ...
        public String toSixFigureString()
        {
            ...
            if (index >= 73)
                index++;
            String secondLetter = Convert.ToString(Convert.ToChar(index));
        ...
        public LatLng toLatLng()
        {
                    ...
                    (b * OSGB_F0)
                        * (((1 + n + ((5.0 / 4.0) * n * n) + ((5.0 / 4.0) * n * n * n)) * (phiPrime - phi0))
                            ...
                            + ((((15.0 / 8.0) * n * n) + ((15.0 / 8.0) * n * n * n))
                                * Math.Sin(2.0 * (phiPrime - phi0)) * Math.Cos(2.0 * (phiPrime + phi0))) - (((35.0 / 24.0) * n * n * n)
                            * Math.Sin(3.0 * (phiPrime - phi0)) * Math.Cos(3.0 * (phiPrime + phi0))));
            ...
            double VIII =
                (Math.Tan(phiPrime) / (24.0 * rho * Math.Pow(v, 3.0)))
                    * (5.0 + (3.0 * Util.tanSquared(phiPrime)) + etaSquared - (9.0 * Util.tanSquared(phiPrime) * etaSquared));
            double IX =
                (Math.Tan(phiPrime) / (720.0 * rho * Math.Pow(v, 5.0)))
                    * (61.0 + (90.0 * Util.tanSquared(phiPrime)) + (45.0 * Util.tanSquared(phiPrime) * Util.tanSquared(phiPrime)));
